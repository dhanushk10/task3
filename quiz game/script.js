...
submitBtn.addEventListener('click', () => {
  ...
  // show feedback
  if (res.correct) {
    showFeedback("Correct ✔ — " + (res.explanation || ""), "correct");
  } else {
    // reveal correct answer text
    const correctText = getCorrectText(questions[currentIndex]);
    showFeedback("Incorrect ✖ — correct: " + correctText + " — " + (res.explanation || ""), "incorrect");
  }
});
...
function formatUserAnswer(q,val) {
  if (val === null || val === undefined || (Array.isArray(val) && val.length === 0)) return "<span style='color:var(--muted)'>No answer</span>";
